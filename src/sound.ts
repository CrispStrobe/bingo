...
export function announce(letter: string, n: number, locale: string, phrase?: string) {
  if (!canSpeak()) return
  const u = new SpeechSynthesisUtterance(phrase ? `${n}. ${phrase}` : letter ? `${letter}. ${n}` : String(n))
  u.lang = locale
